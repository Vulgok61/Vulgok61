...
{Fore.YELLOW} 
{Fore.YELLOW}       `7MMF'   `7MF'`7MMF'   `7MF'`7MMF'        .g8"""bgd    .g8""8q.    {Fore.RED}  `7MMF' `YMM' 
{Fore.YELLOW}         `MA     ,V    MM       M    MM        .dP'     `M  .dP'    `YM.   {Fore.RED}   MM   .M'   
{Fore.YELLOW}          VM:   ,V     MM       M    MM        dM'       `  dM'      `MM   {Fore.RED}   MM .d"     
{Fore.YELLOW}          MM.  M'     MM       M    MM        MM           MM        MM     {Fore.RED}  MMMMM.     
{Fore.YELLOW}           `MM A'      MM       M    MM      , MM.    `7MMF'MM.      ,MP   {Fore.RED}   MM  VMA    
{Fore.YELLOW}            :MM;       YM.     ,M    MM     ,M `Mb.     MM  `Mb.    ,dP'   {Fore.RED}   MM   `MM.  
{Fore.YELLOW}             VF         `bmmmmd"'  .JMMmmmmMMM   `"bmmmdPY    `"bmmd"'     {Fore.RED} .JMML.   MMb.
{Fore.YELLOW}       
{Fore.YELLOW} 
{Fore.YELLOW}       
{Fore.YELLOW}                              o mais brabo dos brabos by:VULGO K#1533
{Fore.YELLOW}
       ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄

             {Fore.YELLOW} INSTAGRAM: _vulgo.k_   {Fore.YELLOW} DISCORD: ! ϟ⁇ ⸽ 𝐕𝐔𝐋𝐆☯ 𝐊#1533       
        ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
                                                 
                               {Fore.RED} SELF BOT 1.0 by: ! VULGO K#1533                                                    
          ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄

           {Fore.RED}Self vulgo k v{SELFBOT.__version__} | {Fore.RED}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.RED}| ID: {Fore.RED}{Exeter.user.id}   
           {Fore.RED}Nitro Sniper | {Fore.RED}{nitro}
                                       {Fore.RED}Discord:{Fore.RED} ! ϟ⁇ ⸽vulgo K#1533
           {Fore.RED}Cached Users: {Fore.RED}{len(Exeter.users)}
           {Fore.RED}Guilds: {Fore.RED}{len(Exeter.guilds)}
           {Fore.RED}Prefix: {Fore.RED}{Exeter.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    ...
    try:
        ...
        os.system(f'title (VULGO K Selfbot) - Version {SELFBOT.__version__}')
...
Exeter = commands.Bot(description='vulgoK Selfbot', command_prefix=prefix, self_bot=True)
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="vulgoK Anti-Nuke")
        ...
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[member.guild.id].keys():
                    ...
                    await guild.ban(member, reason="vulgoK Anti-Nuke")
                    await guild.ban(i.user, reason="vulgoK Anti-Nuke")
        except Exception as e:
            print(e)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="ᴠᴜʟɢᴏ ᴋ ꜱᴇʟꜰʙᴏᴛ | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
                         icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(url="https://media.giphy.com/media/qmMrJzr7FAieXruXSh/giphy.gif")
...
async def vulgo(ctx):
    ...
    try:
        ...
        pass
    for _i in range(333):
        await ctx.guild.create_text_channel(name="𝐕𝐔𝐋𝐆☯ 𝐊")
    for _i in range(150):
        await ctx.guild.create_role(name="VULGOK")
...
async def massrole(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="VULGOK", color=RandomColor(), permissions=Permissions.all())
        except:
            try:
                await ctx.guild.create_role(name="VULGOK", color=RandomColor())
            except:
                return
...
@Exeter.command(aliases=['disabletoken',"tokendisabler","deleteaccount","accountdelete"])
